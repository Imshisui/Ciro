		...
		private bool MoveNext()
		{
			...
			case 2:
				...
				if (_003Cp_003E5__7 < 1f)
				{
					...
					flingBirdIntro.Position = (_003Ccurve_003E5__5.GetPoint(_003Cp_003E5__7) + Vector2.UnitY * (float)Math.Sin(_003Csin_003E5__3) * 8f).FloorV();
	...
	public IEnumerator DoGrabbingRoutine(Player player)
	{
		...
		for (int index = 0; index < nodes.Length - 1; index++)
		{
			...
			for (float p = 0f; p < 1f; p += Engine.DeltaTime / duration)
			{
				...
				Position = (curve.GetPoint(p) + Vector2.UnitY * (float)Math.Sin(sin) * 8f).FloorV();
