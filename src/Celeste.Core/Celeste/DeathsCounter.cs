...
public class DeathsCounter : Component
{
	...
	public Vector2 RenderPosition => Calc.Round(((base.Entity != null) ? base.Entity.Position : Vector2.Zero) + Position);
	...
	public override void Render()
	{
		...
		float num2 = 62f + (float)x.Width + 2f + num;
		Color color = Color;
		...
		if (CenteredX)
		{
			renderPosition -= Vector2.UnitX * (num2 / 2f) * Scale;
		}
		icon.DrawCentered(renderPosition + new Vector2(30f, 0f) * Scale, Color.White * Alpha, Scale * (1f + iconWiggler.Value * 0.2f));
		x.DrawCentered(renderPosition + new Vector2(62f + (float)(x.Width / 2), 2f) * Scale, color, Scale);
		ActiveFont.DrawOutline(sAmount, renderPosition + new Vector2(num2 - num / 2f, (0f - wiggler.Value) * 18f) * Scale, new Vector2(0.5f, 0.5f), Vector2.One * Scale, color, Stroke, black);
	}
}
