...
public class Credits
{
	...
	private class Role : CreditNode
	{
		...
		{
			Font.DrawOutline(FontSize, Name, Calc.FloorV(position), new Vector2(alignment, 0f), Vector2.One * 1.8f * scale, NameColor, 2f, BorderColor);
			position.Y += (LineHeight * 1.8f + 8f) * scale;
			Font.DrawOutline(FontSize, Roles, Calc.FloorV(position), new Vector2(alignment, 0f), Vector2.One * 1f * scale, RolesColor, 2f, BorderColor);
		}
	...
	private class Team : CreditNode
	{
		...
		public override void Render(Vector2 position, float alignment = 0.5f, float scale = 1f)
		{
			Font.DrawOutline(FontSize, Name, Calc.FloorV(position), new Vector2(alignment, 0f), Vector2.One * 1.8f * scale, Role.NameColor, 2f, BorderColor);
			position.Y += (LineHeight * 1.8f + 8f) * scale;
			for (int i = 0; i < Members.Length; i++)
			{
				Font.DrawOutline(FontSize, Members[i], Calc.FloorV(position), new Vector2(alignment, 0f), Vector2.One * 1.4f * scale, TeamColor, 2f, BorderColor);
				position.Y += LineHeight * 1.4f * scale;
			}
			Font.DrawOutline(FontSize, Roles, Calc.FloorV(position), new Vector2(alignment, 0f), Vector2.One * 1f * scale, Role.RolesColor, 2f, BorderColor);
		}
	...
	private class Thanks : CreditNode
	{
		...
		public override void Render(Vector2 position, float alignment = 0.5f, float scale = 1f)
		{
			...
			Font.DrawOutline(FontSize, Title, Calc.FloorV(position), new Vector2(alignment, 0f), Vector2.One * 1.4f * scale, TitleColor, 2f, BorderColor);
			position.Y += (LineHeight * 1.4f + 8f) * scale;
			for (int i = 0; i < Credits.Length; i++)
			{
				if (linkedImages[i] != null)
				{
					MTexture mTexture = GFX.Gui[linkedImages[i]];
					mTexture.DrawJustified(Calc.FloorV(position) + new Vector2(0f, -2f), new Vector2(alignment, 0f), BorderColor, 1.15f * scale, 0f);
					mTexture.DrawJustified(Calc.FloorV(position) + new Vector2(0f, 2f), new Vector2(alignment, 0f), BorderColor, 1.15f * scale, 0f);
					mTexture.DrawJustified(Calc.FloorV(position) + new Vector2(-2f, 0f), new Vector2(alignment, 0f), BorderColor, 1.15f * scale, 0f);
					mTexture.DrawJustified(Calc.FloorV(position) + new Vector2(2f, 0f), new Vector2(alignment, 0f), BorderColor, 1.15f * scale, 0f);
					mTexture.DrawJustified(Calc.FloorV(position), new Vector2(alignment, 0f), CreditsColor, 1.15f * scale, 0f);
					position.Y += (float)mTexture.Height * 1.15f * scale;
				}
				else
				{
					Font.DrawOutline(FontSize, Credits[i], Calc.FloorV(position), new Vector2(alignment, 0f), Vector2.One * 1.15f * scale, CreditsColor, 2f, BorderColor);
	...
	private class MultiCredit : CreditNode
	{
		...
		public override void Render(Vector2 position, float alignment = 0.5f, float scale = 1f)
		{
			...
			for (int i = 0; i < Sections.Length; i++)
			{
				...
				_ = section.Subtitle;
				Font.DrawOutline(FontSize, section.Subtitle, Calc.FloorV(position), new Vector2(alignment, 0f), Vector2.One * 0.7f * scale, SubtitleColor, 2f, BorderColor);
				position.Y += (float)section.SubtitleLines * LineHeight * 0.7f * scale;
				for (int j = 0; j < section.Credits.Length; j++)
				{
					Font.DrawOutline(FontSize, section.Credits[j], Calc.FloorV(position), new Vector2(alignment, 0f), Vector2.One * 1.15f * scale, CreditsColor, 2f, BorderColor);
					position.Y += LineHeight * 1.15f * scale;
				}
		...
		{
			float num = 0f;
			num += (float)TopPadding;
			num += LineHeight * 1.4f + 8f;
			for (int i = 0; i < Sections.Length; i++)
			{
				num += (float)Sections[i].SubtitleLines * LineHeight * 0.7f;
				num += LineHeight * 1.15f * (float)Sections[i].Credits.Length;
			}
			num += 32f * (float)(Sections.Length - 1);
			return num * scale;
		}
	}
	...
	private class ImageRow : CreditNode
	{
		...
		public override void Render(Vector2 position, float alignment = 0.5f, float scale = 1f)
		{
			...
			num2 -= 32f * scale;
			Vector2 vector = position - new Vector2(alignment * num2, 0f);
			array = images;
			foreach (Image image2 in array)
			{
				image2.Render(vector + new Vector2(0f, (num - image2.Height(scale)) / 2f), 0f, scale);
				vector.X += (float)(image2.Atlas[image2.ImagePath].Width + 32) * scale;
			}
		}
	...
	{
		Vector2 position2 = position + Calc.FloorV(new Vector2(0f, 1080f - scroll));
		foreach (CreditNode credit in credits)
		{
